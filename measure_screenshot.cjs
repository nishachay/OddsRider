  ...
  const result = await page.evaluate(async (b64) => {
    return new Promise(resolve => {
      ...
      img.onload = () => {
        ...
        let greenMinX = c.width, greenMaxX = 0;
        ...
        for(let i=0; i<d.length; i+=4) {
          ...
          // green
          if (g > 150 && r > 100 && r < 200 && b < 50) {
            if (x < greenMinX) greenMinX = x;
            if (x > greenMaxX) greenMaxX = x;
          }
        }
        resolve({ imgW: c.width, imgH: c.height, greenWidth: greenMaxX - greenMinX });
      };
      img.src = 'data:image/png;base64,' + b64;
    });
  }, base64);
  console.log("Screenshot metrics:", result);
  await browser.close();
})();
